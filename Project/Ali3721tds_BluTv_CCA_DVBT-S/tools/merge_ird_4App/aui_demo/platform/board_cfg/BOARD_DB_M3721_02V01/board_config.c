...
#include "../../chip/m3702/chip_m3702.h"
...
static gpio_info_t gpio_array[] =
{
	...
	{1, HAL_GPIO_O_DIR, 190},//nim_rest
	{1, HAL_GPIO_I_DIR, 9},//GPIO_KEY_CH_UP
    {1, HAL_GPIO_I_DIR, 8},//GPIO_KEY_CH_DOWN
    {1, HAL_GPIO_I_DIR, 192},//GPIO_KEY_POWER
    {1, HAL_GPIO_O_DIR, 119},//GPIO_LED_RED
    {1, HAL_GPIO_O_DIR, 193},//GPIO_LED_GREEN
#if (defined(NETWORK_SUPPORT) && defined(GPIO_CONTROL_NET_LED))
...
static gpio_info_t demod_gpio_array[] =
{
	{1, HAL_GPIO_O_DIR, DEMOD_GPIO_1},	
};
...
extern INT32 nim_c3501h_attach(struct QPSK_TUNER_CONFIG_API *ptr_qpsk_tuner);
front_end_cfg* front_end_s_cfg(UINT8 tuner_id)
{
	...
	//config i2c pinmux
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 28, PMX_PIN_SEL_I2C);//XI2C2_SDA/TA_28
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 29, PMX_PIN_SEL_I2C);//XI2C2_SCL/TA_29

	PMX_PIN_SET(PMX_GPIO_TA_BASE, 30, PMX_PIN_SEL_GPIO); //NIM_RESET
	//PMX_PIN_SET(PMX_GPIO_TB_BASE, 0, PMX_PIN_SEL_GPIO);
	PMX_PIN_SET(PMX_GPIO_LA_BASE, 8, PMX_PIN_SEL_GPIO); //CH-
	PMX_PIN_SET(PMX_GPIO_LA_BASE, 9, PMX_PIN_SEL_GPIO); //CH+
	PMX_PIN_SET(PMX_GPIO_TB_BASE, 1, PMX_PIN_SEL_GPIO); //GREEN LED
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 15, PMX_PIN_SEL_GPIO);//LNB POWER


	PMX_PIN_SET(PMX_GPIO_TA_BASE, 16, PMX_PIN_SEL_ASSI2);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 17, PMX_PIN_SEL_ASSI2);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 18, PMX_PIN_SEL_ASSI2);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 19, PMX_PIN_SEL_ASSI2);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 20, PMX_PIN_SEL_ASSI2);

#ifdef SUPPORT_TWO_TUNER
	PMX_PIN_SET(PMX_GPIO_RA_BASE, 7, PMX_PIN_SEL_ASSI3);
	PMX_PIN_SET(PMX_GPIO_RA_BASE, 8, PMX_PIN_SEL_ASSI3);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 0, PMX_PIN_SEL_ASSI3);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 1, PMX_PIN_SEL_ASSI3);
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 2, PMX_PIN_SEL_ASSI3);
#endif    
	...
	if(tuner_id == 0)
    {
		cfg->nim_name = "NIM_C3501h_0";		
		cfg->nim_config.s_cfg.attach = nim_c3501h_attach;
		cfg->nim_reset = &gpio_array[NIM_RESET_INDEX];

		cfg->gpio_select = 1; //0:main chip gpio, 1:demod gpio
		cfg->lnb_power = &demod_gpio_array[LNB_CUT_INDEX];

		//qpsk_tuner->ext_lnb_config.int_gpio_select = cfg->gpio_select;
		//qpsk_tuner->ext_lnb_config.int_gpio_num = cfg->lnb_power->position;

		qpsk_tuner->ext_dm_config.i2c_type_id = I2C_TYPE_SCB1;  // 3501 i2c type id
		qpsk_tuner->ext_dm_config.i2c_base_addr = 0x64;
		
		cfg->tsi_cfg.tsi_select = TSI_TS_A;
        cfg->tsi_cfg.tsi_id = TSI_SSI2B_1;
        cfg->tsi_cfg.tsi_attr = 0x5F;//TSI driver will config bit7/bit6/bit5. So it is ok if don't config bit7/bit6/bit5.
		...
		/*
		bit0:M3501_QPSK_FREQ_OFFSET 0: Enable	1: Disable 
		bit1:External or internal ADC select	0: use embedded ADC	1: use external ADC
		bit2:RXADC I/Q SWAP 0: Don't swap I/Q	1: Swap I/Q
		bit3:M3501_I2C_THROUGH 0: diable IIC through	1: enable IIC through
		bit4:H/V polarity	0: H/V polarity NOT revert.	1: H/V polarity revert
		bit5:M3501_NEW_AGC1 0: old agc setting			1: new agc setting
		bit6 bit7:	00: M3501_1BIT_MODE	01: M3501_2BIT_MODE	10: M3501_4BIT_MODE	11: M3501_8BIT_MODE
		*/
		qpsk_tuner->config_data.qpsk_config=0x79;
		
    }
    else
    { 
		cfg->nim_name = "NIM_C3501h_1";		
		cfg->nim_config.s_cfg.attach = nim_c3501h_attach;
		cfg->nim_reset = &gpio_array[NIM_RESET_INDEX];
		...
		qpsk_tuner->ext_dm_config.i2c_type_id = I2C_TYPE_SCB1;  // 3501 i2c type id
		qpsk_tuner->ext_dm_config.i2c_base_addr = 0x24;
		
		cfg->tsi_cfg.tsi_select = TSI_TS_B;
        cfg->tsi_cfg.tsi_id = TSI_SSI2B_2;
        cfg->tsi_cfg.tsi_attr = 0x5F;//TSI driver will config bit7/bit6/bit5. So it is ok if don't config bit7/bit6/bit5.
...
extern INT32 nim_cxd2856_attach_t(char *name, PCOFDM_TUNER_CONFIG_API pConfig);
front_end_cfg* front_end_t_cfg(UINT8 tuner_id)
{
	...
	if(tuner_id == 0)
		...
		cfg->tsi_cfg.tsi_select = TSI_TS_B;
    cfg->tsi_cfg.tsi_spi_tsg_attr = 0x27;//0x27
	t_tuner = &cfg->nim_config.t_cfg.cofdm_tuner;
#if(SYS_DEM_MODULE == CXD2856)		// This flag is not used in Sony NIM(CXD2872+CXD2837)
	t_tuner->config_data.flag = 3; //0:ISDBT_TYPE, 1:DVBT_TYPE, 2:DVBT2_TYPE, 3:DVBT2-COMBO, 4...		
	cfg->nim_name = "NIM_CXD2856_0";		
	cfg->nim_reset = &gpio_array[5];		
	t_tuner->ext_dm_config.i2c_type_id = I2C_TYPE_SCB1;		
	t_tuner->ext_dm_config.i2c_base_addr = 0xC8;		
	cfg->nim_config.t_cfg.attach_with_name = nim_cxd2856_attach_t;		
	t_tuner->ts_mode = NIM_COFDM_TS_SSI;
	/***    setup pinmux obout nim     ***/
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 28, PMX_PIN_SEL_I2C);//XI2C2_SDA/TA_28
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 29, PMX_PIN_SEL_I2C);//XI2C2_SCL/TA_29
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 16, 0x003);//XASSI2_MISTART/XGPIO_TA16
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 17, 0x003);//XASSI2_MIVALA/XGPIO_TA17
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 18, 0x003);//XASSI2_MICLK/XGPIO_TA18
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 19, 0x003);//XASSI2_MDI[0]/XGPIO_TA19
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 20, 0x01);//XASSI2_MDI[1]/XGPIO_TA20
#elif (SYS_DEM_MODULE == COFDM_S3811)
    ...
    t_tuner->tuner_config.i2c_type_id = I2C_TYPE_SCB1;
    t_tuner->tuner_config.c_tuner_base_addr    = 0xC0;
...
void front_end_c_get_qam_config(UINT8 dvbc_mode,UINT32 nim_clk,struct DEMOD_CONFIG_ADVANCED* p_demod_config)
{
    ...
    if(dvbc_mode == DVBC_MODE_J83B)
    {
        ...
        UINT32 qam_buf_size = (256*1024);
        UINT32 *p_qam_buf = NULL;

         p_qam_buf = malloc(qam_buf_size);
         if(p_qam_buf)
        {   
            p_demod_config->qam_buffer_len=  qam_buf_size;
            p_demod_config->qam_buffer_addr= p_qam_buf;
        }
         else
         {
...
{    
	...
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 28, PMX_PIN_SEL_I2C);//XI2C2_SDA/TA_28
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 29, PMX_PIN_SEL_I2C);//XI2C2_SCL/TA_29
	PMX_PIN_SET(PMX_GPIO_TA_BASE, 30, 0x001);//XIF_AGC_QAM/XGPIO_TA30
#else
	 //QAM2
	 if(dvbc_mode == DVBC_MODE_J83B)
...
extern INT32 tun_mxl603_control_DVBC(UINT32 tuner_id, UINT32 freq, UINT32 sym, UINT8 agc_time_const, UINT8 _i2c_cmd);
...
front_end_cfg* front_end_c_cfg(UINT8 tuner_id)
{
    ...
    {//QAM1
        ...
        c_tuner->tuner_config_ext.c_tuner_base_addr    =  0xC0;
        ...
        c_tuner->tuner_config_ext.c_tuner_special_config = 0x01;
        c_tuner->nim_tuner_init = (void *)tun_mxl603_init_DVBC;
        c_tuner->nim_tuner_control = (void *)tun_mxl603_control_DVBC;
        c_tuner->nim_tuner_status = (void *)tun_mxl603_status;
		c_tuner->nim_tuner_command = (void *)tun_mxl603_command;
    }
    else if(tuner_id == 1)
    {//QAM2
...
board_cfg* advance_cfg(board_cfg* cfg)
{
    ...
    cfg->adv_cfg_para.i2c_for_eddc = I2C_TYPE_SCB0;//I2C_TYPE_SCB0;
...
extern INT32 pan_direct_gpio_attach(struct pan_configuration * config);
...
#else
#ifdef SUPPORT_TWO_TUNER
    libc_printf("get board config first nim C\n");
    board_config.front_end_cfg[0] = front_end_c_cfg(0);
#ifdef DVBT_SUPPORT
    libc_printf("get board config second nim 2856T\n");
    board_config.front_end_cfg[1] = front_end_t_cfg(1);
#else defined(ISDBT_SUPPORT)
...
#else
	pan_hw_info.flatch.position = gpio_array[GPIO_KEY_POWER].position;
	pan_hw_info.fclock.position = gpio_array[GPIO_KEY_CH_UP].position;
	pan_hw_info.fdata.position = gpio_array[GPIO_KEY_CH_DOWN].position;
	pan_hw_info.lbd[1].position = gpio_array[GPIO_LED_GREEN].position;
	pan_hw_info.lbd[1].polar = gpio_array[GPIO_LED_GREEN].polar;
	pan_hw_info.lbd[2].position = NULL;//gpio_array[GPIO_LED_RED].position;
	pan_hw_info.lbd[2].polar = NULL;//gpio_array[GPIO_LED_RED].polar;
	
	board_config.pan_i2c_id_set = NULL;
	board_config.pan_i2c_id = 0;
	board_config.pan_attach = pan_direct_gpio_attach;
#endif
