...
ifdef _BUILD_OTA_E_
	ifdef _BUILD_USB_LOADER_RET2GAC
		LSCR= ldscript_3702_usb_ret2gca.ld
	else
	  LSCR = ldscript_3702.ld
  endif
else
LSCR = ldscript_3702.ld
endif
...
SRCS = control.c ctrl_task_init.c ctrl_util.c ap_dynamic_pid.c ctrl_pip.c ctrl_debug.c ctrl_network.c \
       ...
       win_mute.c win_net_local_seting.c win_net_upg.c win_ping.c win_ping_test.c\
       ...
       nit_info.c scpi.c scpi_cmd_drv.c scpi_cmd.c lwip_ping.c win_network.c win_net_choose.c win_net_app.c win_cc.c \
       win_mainmenu_submenu_data.c win_prog_name_extra.c beep_mp2.c win_plsn_task.c
ifdef _DVBS_ENABLE_
...
#ifdef _SUPPORT_256M_MEM
export FLASH_MODULE = BUS_SPI_BUS LLD_STO_SPI_NOR 
#else
#export FLASH_MODULE = LLD_STO_FLASH_SQI
#endif

# OS library
ifndef _BUILD_OTA_E_
OSMOD = ARCH_M63 OS_TDS3 LLD_PMU_M36 PDD_M33 \
		${CORE_MODULE} \
		BUS_EROM_M36\
		VERSION_INFO \
...
ifdef _M36F_SINGLE_CPU
  ifndef _BUILD_OTA_E_
    export DDKLIB = libcore_v40e_m3702_single
    export DDKOSLIB = libcore_v40e_m3702_os_single
  else
    export DDKLIB = libcore_v40e_m3702_single_ota
    export DDKOSLIB = libcore_v40e_m3702_os_single_ota
  endif
  ...
  ifndef _BUILD_OTA_E_
	export DDKLIB = libcore_v40e_m3702
	export DDKOSLIB = libcore_v40e_m3702_os
  ...
  ifdef _BUILD_USB_LOADER_RET2GAC
      ...
      export DDKLIB = libcore_v40e_m3702_usb_ret2gac
      export DDKOSLIB = libcore_v40e_m3702_os_usb_ret2gac
    ...
    ifdef _BUILD_USB_LOADER_
  		ifdef _BUILD_USB_LOADER_GEN
  	  		  export DDKLIB = libcore_v40e_m3702_usb
         		export DDKOSLIB = libcore_v40e_m3702_os_usb
         	else
         		export DDKLIB = libcore_v40e_m3702
         		export DDKOSLIB = libcore_v40e_m3702_os
         	endif
	...
	ifdef _CAS9_CA_ENABLE_
		ifdef _CAS9_VSC_ENABLE_
			ifdef _CAS9_VSC_RAP_ENABLE_
				...
				ifdef _CAS9_SC_UPG_
					export CA_LIB = libconax_scu.a libupg_rs232_3702_vsc.a
				else
					export CA_LIB = libconax_vsc.a libupg_rs232_3702_vsc.a
				endif
			endif
		else
		export CA_LIB = libconax.a libupg_rs232_3702.a
		endif
		export FONT_LIB = $(DDK_DIR)/libfreetype.a
	else
                ifdef _BC_CA_ENABLE_
...
export CHIP_CFG_PATH = platform/chip/m3702
export CHIP_AVDCFG_PATH = platform/advanced_cfg/m3702_board

ifdef _BOARD_DB_M3712L_01V01_
export BOARD_CFG_PATH = platform/board_cfg/BOARD_DB_M3712L_01V01
endif
ifdef _BOARD_DB_M3721L_01V01_
export BOARD_CFG_PATH = platform/board_cfg/BOARD_DB_M3721L_01V01
endif
ifdef _BOARD_DB_M3721_02V01_
export BOARD_CFG_PATH = platform/board_cfg/BOARD_DB_M3721_02V01
endif
ifdef _BOARD_DB_M3729_01V01_
export BOARD_CFG_PATH = platform/board_cfg/BOARD_DB_M3729_01V01
endif
...
ifdef _HTTPS_ENABLE_
	cp -f  ${BASE_DIR}/ddk/libopenssl/libssl.a ${LIB_DIR}
	cp -f  ${BASE_DIR}/ddk/libopenssl/libcrypto.a ${LIB_DIR}
endif
ifdef _OPENSSL_ENABLE
...
#	cp -f ../${ROOT}/${SEEDIR}/src/see/m36f/compiler_cpu.def ./
	cp -f ../${ROOT}/${SEEDIR}/ddk/${SEELIB}*.a ${ROOT}/ddk;
	cp -f ../${ROOT}/${SEEDIR}/ddk/${BOOT_SEE}.o ${ROOT}/ddk/;
#	cp -f ../${ROOT}/${SEEDIR}/ddk/plugin/* ${ROOT}/ddk/plugin/
#*/
...
#	mkdir -p ${DDK_REL_DIR}/${DDK_PRJ_DIR}/loader3_AS_G2
	mkdir -p ${DDK_REL_DIR}/${DDK_PRJ_DIR}/DDR-Setting/C3702
...
ifdef _C1800A_CA_ENABLE_
	cp -f ${DDK_DIR}/libimg_eng_v40b*.a ${DDK_REL_DIR}/ddk/
endif
...
ifdef DUAL_ENABLE
	cp ${DDK_DIR}/libsee_m3702*.a	${DDK_REL_DIR}/ddk/
endif
	cp ${DDK_DIR}/libcore_v40e_m3702*.a	${DDK_REL_DIR}/ddk/
	cp ${DDK_DIR}/libplus_v40b*.a	${DDK_REL_DIR}/ddk/
...
#	cp ${DDK_DIR}/plugin/lib_ogg.o ${DDK_REL_DIR}/ddk/plugin/
	...
	#cp ${DDK_DIR}/boot_see_m3702.o ${DDK_REL_DIR}/ddk/
	cp ${DDK_DIR}/libHDMI.a ${DDK_REL_DIR}/ddk/
	cp -f ${DDK_DIR}/${C_STDBY}.a ${DDK_REL_DIR}/ddk/
	
ifndef _BUILD_OTA_E_	
	cp ${DDK_DIR}/libhttpmgserver.a ${DDK_REL_DIR}/ddk/
	#release live555 lib
	cp -f ${DDK_DIR}/liblive555*.a ${DDK_REL_DIR}/ddk/
	cp -f ${DDK_DIR}/libsupc++.a ${DDK_REL_DIR}/ddk/
	cp -f ${DDK_DIR}/libjson.a	${DDK_REL_DIR}/ddk/
	#release youtube
#	mkdir -p ${DDK_REL_DIR}/inc/api/libyoutube
	mkdir -p ${DDK_REL_DIR}/inc/openssl/
#	mkdir -p ${DDK_REL_DIR}/inc/api/libyoutube/
#	cp -f ${DDK_DIR}/libyoutube.a ${DDK_REL_DIR}/ddk/
...
#	cp -r loader3_AS_G2 ${DDK_REL_DIR}/${DDK_PRJ_DIR}/ ;echo
	...
	cp -rf ${BASE_DIR}/inc/bus/spi ${DDK_REL_DIR}/inc/bus
	
#	mkdir -p ${DDK_REL_DIR}/src/lld/sto/spi_nor
	mkdir -p ${DDK_REL_DIR}/src/bus/spi
#	cp -rf ${BASE_DIR}/src/lld/sto/spi_nor/* ${DDK_REL_DIR}/src/lld/sto/spi_nor
	cp -rf ${BASE_DIR}/src/bus/spi/* ${DDK_REL_DIR}/src/bus/spi
	...
	cp -f ./ldscript_3702*.ld ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./see_ldscript_M3702*.ld ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./compiler_3702.def ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./sys_config_3702.h ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./sys_memmap_3702*.h ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	...
	cp -f ./universal_key_encrypt_C3702.sh ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./ota_link.c ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./how_to_compile_3702*.txt ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	...
	cp -f ./37*ouc_loader.bin ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	...
	cp -f ./3702_ret_bl_gen.sh	${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -rf ./bloader_cmd		${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -f ./root*.c ${DDK_REL_DIR}/${DDK_PRJ_DIR}/
	cp -rf ./DDR-Setting/C3702/Golden ${DDK_REL_DIR}/${DDK_PRJ_DIR}/DDR-Setting/C3702/
...
ifeq ($(FSC_APP_DIR), fsc_ap)
  ...
  # for temp cxd2856 rel
	...
	rm -f ${DDK_REL_DIR}/ddk/libsee*.a
	rm -f ${DDK_REL_DIR}/ddk/plugin/*.o
	rm -f ${DDK_REL_DIR}/ddk/plugin/*.bak
	
	rm -f ${DDK_REL_DIR}/ddk/lib*m32.a
	...
	-rm -f ${DDK_REL_DIR}/ddk/libplus_*open.a
...
cas9_vsc_rel:
	...
	cp -f ${BASE_DIR}/${DDK_PRJ_DIR}/conax_ap/C3702_*.txt ${CAS9_VSC_REL_DIR}/${DDK_PRJ_DIR}/conax_ap/	
...
c1800a_ca_rel:    
	mkdir -p ${C1800A_CA_REL_DIR}/ddk
	mkdir -p ${C1800A_CA_REL_DIR}/ddk/blsdk
	...
	cp -f ${BASE_DIR}/ddk/libfreetype.a ${C1800A_CA_REL_DIR}/ddk/
	...
	rm -rf ${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/ird_ap/DVB_SUIT
	rm -rf ${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/ird_ap/TEMU
	
	cp -f compiler_3702.def ${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/
	#enable gen macro
	sed -i "s/_GEN_CA_ENABLE_ = 1/#_GEN_CA_ENABLE_ = 1/g" 	${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
	sed -i "s/#_C1800A_CA_ENABLE_ = 1/_C1800A_CA_ENABLE_ = 1/g" 	${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
	sed -i "s/#_USE_SIGNED_BL_SEE = 1/_USE_SIGNED_BL_SEE = 1/g" 	${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
	sed -i "s/#_AUI_ = 1/_AUI_ = 1/g" 	${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
	sed -i "s/#_AUI_TEST_ = 1/_AUI_TEST_ = 1/g" 	${C1800A_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
...
gen_ca_rel:
	...
	cp -f compiler_3702.def ${GEN_CA_REL_DIR}/${DDK_PRJ_DIR}/
	#enable gen macro
	sed -i "s/_CAS9_CA_ENABLE_ = 1/#_CAS9_CA_ENABLE_ = 1/g" 	${GEN_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
	sed -i "s/#_GEN_CA_ENABLE_ = 1/_GEN_CA_ENABLE_ = 1/g" 	${GEN_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
	sed -i "s/#_USE_SIGNED_BL_SEE = 1/_USE_SIGNED_BL_SEE = 1/g" 	${GEN_CA_REL_DIR}/${DDK_PRJ_DIR}/compiler_3702.def
...
special_config:
	cp -f sys_memmap_3702.h ${ROOT}/inc/sys_memmap.h
