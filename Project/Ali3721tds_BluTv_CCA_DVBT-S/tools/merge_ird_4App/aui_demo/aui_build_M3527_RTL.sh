...
build_signature_firmware()
{
	...
	# Go to the correct directory: RTL only generate firmware at aui_demo, and no need signature
	cd ${PROJECT_APP}

	# RTL no need to signature
	make bin
	check_previous_return_val
}
...
backup_firmware()
{
	...
	cp ${PROJECT_ROOT}/obj/product_sabbat_dual.abs ${PROJECT_ROOT}/images/product_sabbat_dual_aui_m3527_rtl.abs
	check_previous_return_val
}
