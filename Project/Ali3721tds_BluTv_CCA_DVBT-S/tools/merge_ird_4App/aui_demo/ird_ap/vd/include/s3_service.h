/**
 * $Id: s3_service.h,v 1.78 2010/08/05 23:27:10 jcao Exp $
 *
...
#define IA_INVALID_DMB_CAMODE       0xFF
#define IA_INVALID_DMB_SUBCHID      0xFF
#define IA_INVALID_DMB_PA           0xFFFF
#define IA_INVALID_DMB_TRID         0xFFFF
...
enum
{
	...
	RESOURCE_DMB_ECM_V2             = 31,
	RESOURCE_FLO_ECM                = 32,
	RESOURCE_FLO_EMM                = 33,
	RESOURCE_FLO_DESCRAMBLER        = 34,
	RESOURCE_SESSION_MANAGEMENT_CCA_METADATA        = 35
};
...
typedef struct
{
	...
	ia_word16			wEcmPid;
	ia_status_st         stStatus;
	vd_rsc_id			wResourceId;/*smartcard resource id*/
	ia_word16			wMatchedSCount;/*Number of matched cards*/
} msg_gmss_stream_status_st;
...
typedef struct
{
	...
	ia_byte              bContentSubChId;
	ia_word16          wContentPA;
	ia_word16          wContentTransportId;
} msg_dmb_content_streamid_v2_st;
...
typedef struct
{
	...
	ia_bool				fHasKmsId;
	ia_word32			wKmsId;
	ia_word32			wFLOStreamId;
	ia_byte			abSalt[ 8 ];
	vd_ip_domain_type_e  eDomain;
	ia_bool				fHasEcmStream;
	ia_status_st         stStatus;
} msg_flo_content_stream_status_st;
	/* Structure is followed by Content_IP_Socket_Address, its type depending on eDomain.
		Structure is then followed by ECM_IP_Socket_Address, its type depending on eDomain and only
		if fHasEcmStream is IA_TRUE. */
...
/* Strongly consider following the above CMMB style monitoring sturcture for FLO! */
typedef struct
{
	ia_word16          wServiceHandle;
	vd_rsc_id           wResourceId;
...
typedef struct 
{
	ia_word16	wServiceHandle;
	ia_byte	bFlag;
} msg_dtcp_ip_flag_notify_st;

/*DTCP IP flag absence support*/
typedef struct 
{
	ia_word16	wServiceHandle;
} msg_dtcp_ip_absence_notify_st;


/*is-shareable flag support*/
typedef struct 
{
	ia_word16		wServiceHandle;
	ia_bool			fIsShareable;
} msg_whn_is_shareable_flag_notify_st;

typedef struct
{
	ia_word32		wRecordId;
}msg_whn_is_shareable_flag_query_st;

typedef enum
{
	IS_SHAREABLE_FLAG_QUERY_SUCCESS = 1,
	IS_SHAREABLE_FLAG_QUERY_NOT_EXIST,
	IS_SHAREABLE_FLAG_QUERY_RECORD_ERROR
}is_shareable_flag_query_result_e;

typedef struct
{
	is_shareable_flag_query_result_e eResult;
	ia_bool fIsShareable;
}msg_whn_is_shareable_flag_reply_st;


/*device domain id support*/
typedef struct 
{
	ia_byte     bSectorNumber;
	ia_word32	wDeviceDomainID;
}msg_whn_device_domain_id_notify_st;

typedef enum
{
	DDID_QUERY_SUCCESS = 1,
	DDID_QUERY_NOT_EXIST
} device_domain_id_query_result_e;

typedef struct
{
	ia_byte     bSectorNumber;
	ia_word32	wDeviceDomainID;
}msg_device_domain_id_info_st;

typedef struct
{
	ia_byte	bSectorCount;
	device_domain_id_query_result_e eResult;
} msg_whn_device_domain_id_reply_st ;

/*support enhanced copy protection notification*/
typedef struct
{
	ia_word16		wServiceHandle;
	...
	ia_byte		bLength;
	ia_byte		*pbData;
}msg_enhanced_copy_protection_notify_st;

/*support trick mode notification*/
typedef struct
{
	ia_word16		wServiceHandle;
	ia_word16		wStreamPid;
