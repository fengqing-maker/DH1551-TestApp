...
COMPILER_DEF=${PROJECT_APP}/"compiler_3702.def"
COMPILER_DEF_SABBAT=${PROJECT_APP_SABBAT}/"compiler_3702.def"

SYS_CONFIG_3702=${PROJECT_APP}/"sys_config_3702.h"
...
build_main_config()
{
	...
	option_select "enable" "_BOARD_M3721_02V01_ = 1" ${COMPILER_DEF}
	option_select "disable" "_C1800A_LOADERCORE_ENABLE_ = 1" ${COMPILER_DEF}
	option_select "disable" "_C1800A_DVT_TEST_ = 1" ${COMPILER_DEF}
	...
	option_select "disable" "_BOARD_M3712L_01V01_ = 1" ${COMPILER_DEF}
	...
	option_select "enable" "_C1800A_CA_ENABLE_ = 1" ${COMPILER_DEF}
	# Now ensure all AS options are disabled
	option_select "disable" "_GEN_CA_ENABLE_ = 1" ${COMPILER_DEF}
	...
	echo "sed -i \"s/^#undef SMC1_SUPPORT/#define SMC1_SUPPORT/\" ${SYS_CONFIG_3702}"
	eval "sed -i \"s/^#undef SMC1_SUPPORT/#define SMC1_SUPPORT/\" ${SYS_CONFIG_3702}"
	check_previous_return_val

	echo "sed -i \"s/^#undef SMC2_SUPPORT/#define SMC2_SUPPORT/\" ${SYS_CONFIG_3702}"
	eval "sed -i \"s/^#undef SMC2_SUPPORT/#define SMC2_SUPPORT/\" ${SYS_CONFIG_3702}"
	check_previous_return_val

	echo "sed -i \"s/^\/\/#define SMC1_SUPPORT/#define SMC1_SUPPORT/\" ${SYS_CONFIG_3702}"
	eval "sed -i \"s/^\/\/#define SMC1_SUPPORT/#define SMC1_SUPPORT/\" ${SYS_CONFIG_3702}"
	check_previous_return_val

	echo "sed -i \"s/^#define SUPPORT_TWO_TUNER/\/\/#define SUPPORT_TWO_TUNER/\" ${SYS_CONFIG_3702}"
	eval "sed -i \"s/^#define SUPPORT_TWO_TUNER/\/\/#define SUPPORT_TWO_TUNER/\" ${SYS_CONFIG_3702}"
	check_previous_return_val
	
	# Now use the config
	make path config_3702
	check_previous_return_val
...
build_signature_firmware()
{
	...
	# Go to the correct directory: 3702 only generate firmware at aui_demo, and signature
	cd ${PROJECT_APP}
...
backup_firmware()
{
	...
	cp ${PROJECT_ROOT}/obj/product_sabbat_dual.abs ${PROJECT_ROOT}/images/product_sabbat_dual_aui_m3721_02v01_gen.abs
	check_previous_return_val
}
