...
#include "LoaderCoreSPI_TraceLog.h"

#ifndef SIGNATURE_LENGTH
#define SIGNATURE_LENGTH    (256) //loader 5.1

#endif 
#define LC_BIT_ASSERT_BASE(condition, arg1, arg2, arg3) { \
    if (!(condition)) { \
...
#define LC_BIT_ASSERT_ERROR                ("&Assert check failed at %s:%d\n")
#define LC_BIT_ASSERT(condition)     LC_BIT_ASSERT_BASE(condition, LC_BIT_ASSERT_ERROR, __FILE__, __LINE__)
...
lc_result MessageParser_ReadBits(const lc_buffer_st *pBytes, lc_uint32 *pBitPos, lc_uint32 bitlength,void *pDest)
{
    ...
    lc_serial_download_packet *pParsed)
{
    ...
    LoaderCoreSPI_Stdlib_memset(&datagram, 0, sizeof(lc_module_dg_st));
    ...
    const lc_module_info_header *pParsed)
{
    ...
    LoaderCoreSPI_Stdlib_memcpy(&parsed, pParsed, sizeof(lc_module_info_header));
    ...
    if (LC_SUCCESS == result)
    {
        LC_BIT_ASSERT((lc_uint16)((*pBitPos - startingBitPos) / 8) 
                        == pParsed->header_length);
    }
    ...
    lc_bbcb_st *pParsed)
{
    ...
    if (LC_SUCCESS == result)
    {
		...
		TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB> hardware_version = %d\n", 
            pParsed->hardware_version);
    }
    ...
    if (LC_SUCCESS == result)
    {
		...
		TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB>loader_gte_v5 = %u\n", 
            pParsed->loader_major_version);
    ...
    if (LC_SUCCESS == result)
    {
		...
		TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB> Loader minor version = %d\n", 
            pParsed->loader_minor_version);
    }
    ...
    if (LC_SUCCESS == result)
    {
		...
		TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB>loader_version_buildnumber = 0x%d\n", 
            pParsed->loader_version_buildnumber);
    }
    ...
    if (LC_SUCCESS == result)
    {
	    ...
	    TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB>boot_check_algorithm = 0x%x\n", 
            pParsed->boot_check_algorithm);
    }
    ...
    if (LC_SUCCESS == result)
    {
		...
		TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB> serial_number = %s\n", 
            pParsed->serial_number);
    }
    ...
    if (LC_SUCCESS == result)
    {
		...
		TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB> random_number = %s\n", 
           pParsed->random_number);
    }
    ...
    if (LC_SUCCESS == result)
    {
	    ...
	    TRACE_LOG1(
            TRACE_DEBUG, 
            "[PARSER] <BBCB>pParsed->crc32 = 0x%4x\n", 
             pParsed->crc32);
    }
    ...
    lc_ota_dl_para_st *pParsed)
{
    ...
    LoaderCoreSPI_Stdlib_memcpy(&parsed, pParsed, sizeof(lc_ota_dl_para_st));
    ...
    lc_shared_memory_st *pParsed)
{
    ...
    LoaderCoreSPI_Stdlib_memcpy(&parsed, pParsed, sizeof(lc_shared_memory_st));
    ...
    lc_last_download_info_st *pParsed)
{
    ...
    LoaderCoreSPI_Stdlib_memcpy(&parsed, pParsed, sizeof(lc_last_download_info_st));
...
void LC_Free_Memory( void **ppMem )
{  
    if( (LC_NULL != ppMem) &&  (LC_NULL != *ppMem)) 
    {
...
void* LC_MALLOC(lc_uint32 bufferSize)
{
    ...
    LC_BIT_ASSERT(pBuffer);
    if(pBuffer)
    {
        /*Init it.*/
        LoaderCoreSPI_Stdlib_memset(pBuffer, 0x0, bufferSize);
    }
    else
    {
