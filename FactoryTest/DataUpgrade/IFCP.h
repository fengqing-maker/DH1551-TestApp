...
public:
	unsigned int IFCPInit(unsigned char *pcChipID, int nLen);
	unsigned int GetFileName(unsigned char *cChipID, int nLen);
...
private:
	unsigned char m_ucIFCPHexData[64 * 1024];
	...
	unsigned char m_ucIFCP_Image[64 * 1024];

	File_Info_S m_stSTHFile;

	unsigned char m_acIFCPFileName[256];
	unsigned char m_acLastChipID[8];
};
