...
#define IFCP_BUFFER_SIZE (64*1024)
...
void SerialUpgrade::Init(SNConfig csSNConfig)
{
	...
	m_WriteType += m_SNConfig.m_nIFCP * E_IFCP_WRITE;
...
void SerialUpgrade::StartWriteFlash(void)
{
	...
	do
	{
		...
		if(m_WriteType & E_PEF_WRITE)
		{
			...
			int nPefRet = WritePEF(aucPEF, unPEFLen);
	...
	if(unFinish > 0)
	{
		/*if((m_WriteType & E_CONAX_PERSONALIZE) == E_CONAX_PERSONALIZE)
		{
			m_nStatus = WAIT_CONAX_PERSONALIZE;
		}
		else if ((m_WriteType & E_IFCP_WRITE) == E_IFCP_WRITE)
		{
			m_nStatus = WAIT_IRDETO_IFCP;
		}
		else
		{
		...
		SaveUpgradeStatus();
		//m_pUpgradeNotify->UpgradeSuccess();
		m_SerialBuf.WriteLog("*************m_nStatus %d*************\r\n", m_nStatus);
	}
}

#define SEND_ONCE_SIZE (5*1024)

void SerialUpgrade::WriteIrdetoIFCP(void)
{
	...
	do
	{
		...
		if(GetChipID(&stChipID) == 0)
		{
			m_SerialBuf.WriteLog("*************Read ChipID Fail*************\r\n");
			break;
		}
		unSendLen = stChipID.m_nLength - 4;
		memcpy(ucSendData, &stChipID.m_szChipID[4], unSendLen);	
		
		if ( SendAndReceiveUartData(E_UPGRADE_COMMAND_STB_REQ_IFCP_SIZE, E_UPGRADE_COMMAND_PC_SEND_IFCP_SIZE, ucSendData, unSendLen, 2*1000) == 0 )
		{
			m_SerialBuf.WriteLog("*************Request ifcp size Fail*************\r\n");
			break;
		}
#if 1
		{
			pbuf = new unsigned char[IFCP_BUFFER_SIZE];
			
			if ( pbuf == NULL )
			{
				m_SerialBuf.WriteLog("*************Request New BufferSize:0x%x Fail*************\r\n", unSize);
				break;
			}
			memset(pbuf, 0, IFCP_BUFFER_SIZE);
			unSize = m_IFCP.ParseAllData( ucSendData, pbuf, IFCP_BUFFER_SIZE);
			
			m_SerialBuf.WriteLog("*************m_IFCP BufferSize:0x%x*************\r\n", unSize);
		}
		if(unSize == 0)
		{
			break;
...
#if 0 
		//1、request ifcp size
		if ( SendAndReceiveUartData(E_UPGRADE_COMMAND_STB_REQ_IFCP_SIZE, E_UPGRADE_COMMAND_PC_SEND_IFCP_SIZE, ucSendData, unSendLen, 15*1000) == 0 )
		{
			m_SerialBuf.WriteLog("*************Request ifcp size Fail*************\r\n");
			break;
		}

		if ( m_CommandData.m_nDataLen < 4 )
		{
			m_SerialBuf.WriteLog("*************Request ifcp size info Fail, len=%d*************\r\n", m_CommandData.m_nDataLen);
			break;
		}

		unSize |= (unsigned int)(m_CommandData.m_aucData[0] << 24);
		unSize |= (unsigned int)(m_CommandData.m_aucData[1] << 16);
		unSize |= (unsigned int)(m_CommandData.m_aucData[2] << 8);
		unSize |= (unsigned int)(m_CommandData.m_aucData[3]);
		pbuf = new unsigned char[unSize];
		if ( pbuf == NULL )
		{
			m_SerialBuf.WriteLog("*************Request New BufferSize:0x%x Fail*************\r\n", unSize);
			break;
		...
		//2、request ifcp data
		unsigned int unSendIndex = 0;
		unsigned int unRcvIndex = 0;
		unsigned int unBufIndex = 0;
		unsigned int unSegCount = unSize / SEND_ONCE_SIZE;
		if (unSize % SEND_ONCE_SIZE > 0)
		{
			unSegCount += 1;
		}
		while ( unSendIndex < unSegCount )
		{
			unSendLen = 2;
			ucSendData[0] = (unsigned char)((unSendIndex >> 8) & 0xff);
			ucSendData[1] = (unsigned char)(unSendIndex & 0xff);
			if ( SendAndReceiveUartData(E_UPGRADE_COMMAND_STB_REQ_IFCP+unSendIndex, E_UPGRADE_COMMAND_PC_SEND_IFCP+unSendIndex, ucSendData, unSendLen, 20*1000) == 0 )
			{
				m_SerialBuf.WriteLog("*************Request ifcp data Fail*************\r\n");
				break;
			}
			if (m_CommandData.m_nDataLen <= 2)
			{
				m_SerialBuf.WriteLog("*************Request ifcp size Less than 3 Error, len=%d*************\r\n", m_CommandData.m_nDataLen);
				break;
			}
			unRcvIndex = (unsigned int)(((m_CommandData.m_aucData[0] << 8) | m_CommandData.m_aucData[1]));
			if ( unRcvIndex != unSendIndex )
			{
				m_SerialBuf.WriteLog("*************Request ifcp size Seg Error:Rcv=%d, Snd=%d*************\r\n", unRcvIndex, unSendIndex);
				continue;
			}
			memcpy( &pbuf[unBufIndex], &m_CommandData.m_aucData[2], m_CommandData.m_nDataLen - 2);
			unBufIndex += (m_CommandData.m_nDataLen - 2);
			unSendIndex++;
		}

		if ( unSendIndex != unSegCount || unBufIndex != unSize )
		{
			m_SerialBuf.WriteLog("******Request ifcp Error, unSendIndex=%d, unSize=0x%x, unBufIndex=0x%x******\r\n", unSendIndex, unSize, unBufIndex);
			break;
		}
#endif
		int nIFCPRet = WriteIFCP( pbuf, unSize );
		m_SerialBuf.WriteLog("[%s][%d]WriteIFCP nPefRet:%d\n", __FUNCTION__, __LINE__, nIFCPRet);
		if (nIFCPRet != E_DATA_SUCCESS)
		{
			m_SerialBuf.WriteLog("[%s][%d]WriteIFCP Fail\n", __FUNCTION__, __LINE__);
			break;
		}
		if ( SendAndReceiveUartData(E_UPGRADE_COMMAND_STB_SEND_IFCP_RESULT, E_UPGRADE_COMMAND_PC_SEND_IFCP_RESULT, NULL, 0, 0) == 0 )
		{
			m_SerialBuf.WriteLog("*************Request ifcp data Fail*************\r\n");
			break;
		}
