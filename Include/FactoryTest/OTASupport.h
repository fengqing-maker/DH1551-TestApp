...
private:
	...
	unsigned int FlashCCACoreFileByUSB ( unsigned int unChipID );
	unsigned int FlashCCACoreFile(unsigned int unChipID);
	unsigned int FlashCFGFileByUSB();
	...
	unsigned int GetUSBRootPath(char *pBuffer, int nPathLen);
	unsigned int GetUSBDataByName ( unsigned char *pFileName, unsigned int unFileLen, unsigned char * pucDataBuf, unsigned int unDataBufLen);
