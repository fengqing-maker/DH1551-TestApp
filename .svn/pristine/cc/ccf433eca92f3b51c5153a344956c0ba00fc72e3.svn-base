...
build_main_config()
{
	...
	option_select "enable" "_BOARD_M3527_01V01_ = 1" ${COMPILER_DEF}
	...
	option_select "enable" "_CAS9_CA_ENABLE_ = 1" ${COMPILER_DEF}
	option_select "enable" "_CAS9_VSC_ENABLE_ = 1" ${COMPILER_DEF}
	...
	option_select "enable" "_ENABLE_BL_ENCRYPTED_ = 1" ${COMPILER_DEF}
...
# This function use to configure the project, to use signed SEE  features etc.
build_main_config_enable_signed_see()
{
	echo "AUI_BUILD_STEP: build_main_config"
	# Go to the correct directory
	cd ${PROJECT_APP}
	
	# Ali internal version, should not use signed see
	# option_select "enable" "_USE_SIGNED_BL_SEE = 1" ${COMPILER_DEF}
...
backup_firmware()
{
	...
	cp ${PROJECT_ROOT}/obj/product_sabbat_dual.abs ${PROJECT_ROOT}/images/product_sabbat_dual_aui_m3527_vsc.abs
	check_previous_return_val
}
...
# For 3505 VSC project, ALI internal use
build_special_plugin_enable()
{
	cd ${PROJECT_ROOT}/ddk/plugin/t2
	cp plugin_eac3_3505.o.bak plugin_eac3_3505.o
	check_previous_return_val
}
